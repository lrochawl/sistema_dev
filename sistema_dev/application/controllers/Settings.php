...
class Settings extends MY_Controller
{
    ...
    public function getSettings($id)
    {
        ...
        foreach ($this->data['results'] as $settings) {
            ...
            if ($this->permission->checkPermission($this->session->userdata('permissao'), 'vProduto')) {
                $v = '<a style="margin-right: 1%" href="' . site_url() . "settings/visualizar/".$id ."/" . $settings->{"id_estoque_".$this->data['id'] } . '" class="btn-nwe" title="Visualizar Setting"><i class="bx bx-show bx-xs"></i></a>  ';
            }
